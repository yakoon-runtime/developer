...
Files generated by `yak` (e.g. `.yak/context.toml` from `yak init`) document
their top-level model concepts with a short English comment explaining the
concept's semantic role (not its technical implementation). Not every
property needs a comment — keep obvious entries sparse. Comments are
produced by the generator, never by hand.
